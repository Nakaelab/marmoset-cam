...
function create3DMonitor(texture) {
  ...
  // Position the monitor at the back of the cage (+X), positioned high near top of green wall
  if (controls && controls.target) {
    group.position.set(
      controls.target.x + 0.85, // Shift back (+X)
      controls.target.y + 0.50, // High position near top of background wall (+Y)
      controls.target.z
    );
  } else {
    group.position.set(0.85, 1.35, 0.11);
  }
